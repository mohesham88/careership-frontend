...
export default function TeamDetail() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([fetchTeam(uuid!), fetchInvitations(uuid!)])
      .then(([teamRes, invRes]) => {
  ...
  return (
    <Container maxWidth="md" sx={{ py: 6 }}>
              ...
              <Box key={inv.uuid} sx={{ mb: 1 }}>
                <Chip
                  label={`Created: ${new Date(
                    inv.created_at
                  ).toLocaleDateString()} | Expires in: ${
                    inv.expires_in_days
                  } days | Active: ${inv.is_active ? "Yes" : "No"}`}
                  color={inv.is_active ? "success" : "default"}
                  variant="outlined"
                  onClick={() =>
                    inv.is_active ? handleDisable(inv) : handleEnable(inv)
                  }
                  clickable
                />
                {inv.invitation_url && (
      ...
      <Dialog
        ...
        onClose={() => setConfirmRemove({ open: false, email: null })}
      >
        ...
        <DialogActions>
          <Button
            onClick={() => setConfirmRemove({ open: false, email: null })}
          >
            Cancel
          </Button>
          <Button
      ...
      <Dialog
        ...
        onClose={() => setConfirmDelete({ open: false, uuid: null })}
      >
        ...
        <DialogActions>
          <Button onClick={() => setConfirmDelete({ open: false, uuid: null })}>
            Cancel
          </Button>
          <Button
            color="error"
